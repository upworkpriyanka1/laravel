...
class Registered_nurse_admin extends CI_Controller {
	...
	/*check if allowed to access page */
		 $group = array('registered-nurse-admin'); //allowed group admin,super etc
		 if (!$this->ion_auth->in_group($group)){
			 echo "Not allowed";
			 die();
		 }
	/* load library & model with aliases, config and language */
			$this->load->library('Registered_nurse_admin_lib',NULL, 'nurse_lib');
			$this->load->model('registered_nurse_admin_mdl','nurse_mdl');
			$this->lang->load('registered_nurse_admin');
			$this->config->load('registered_nurse_admin_menu', true );
			$this->menu    			= $this->config->item( 'registered_nurse_admin_menu' );
