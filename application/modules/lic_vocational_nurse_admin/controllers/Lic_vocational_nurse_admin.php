...
class Lic_vocational_nurse_admin extends CI_Controller {
	public function __construct() {
	   ...
	   /*check if logged in */
		...
		/*check if allowed to access page */
			$group = array('lic-vocational-nurse-admin'); //allowed group admin,super etc
			if (!$this->ion_auth->in_group($group)){
				echo "Not allowed";
				die();
			}
	/* load library & model with aliases, config and language */
			$this->load->library('Lic_vocational_nurse_admin_lib',NULL, 'nurse_lib');
			$this->load->model('lic_vocational_nurse_admin_mdl','nurse_mdl');
			$this->lang->load('lic_vocational_nurse_admin');
			$this->config->load('lic_vocational_nurse_admin_menu', true );
			$this->menu    			= $this->config->item( 'lic_vocational_nurse_admin_menu' );
