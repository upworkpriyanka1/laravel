...
/**********************
 * clicking on "Filter" button in vendors View page filters popup dialog is opened and inputs are filled from "hidden_" hidden inputs of form and date initialization
 * access public
 * return none
 *********************************/
function vendorsListFilterApplied( ) {
    $('.tooltip-inner').css('display', 'none');

    $( "#vendors_list_dialog_filter" ).modal(  {
        "backdrop": "static",
        "keyboard": true,
        "show": true
    }  );
    $("#filter_vn_name").val( jQuery.trim($("#hidden_filter_vn_name").val()) )
 ...
 * return none
 *********************************/
function vendorsListMakeFilterDialogSubmit() {
    $("#hidden_filter_vn_name").val( jQuery.trim($("#filter_vn_name").val()) )
    ...
    $("#page_number").val(1)
    var theForm = document.getElementById("form_vendors");
    theForm.submit();
}
