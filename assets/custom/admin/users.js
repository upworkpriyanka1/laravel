    ...
    success: function (result) {
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].first_name) {
                searchFields.push({"id": data[i].id, "value": data[i].first_name});
            }
            if (data[i].last_name) {
                searchFields.push({"id": data[i].id, "value": data[i].last_name});
            }
            if (data[i].username) {
                var str = data[i].username;
                searchFields.push({"id": data[i].id, "value": str.trim()});
            }
            if (data[i].email) {
                searchFields.push({"id": data[i].id, "value": data[i].email});
            }
        }
    }
});
