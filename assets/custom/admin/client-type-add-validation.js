...
function post_form(){
    ...
    var formURL = $('#client-type-add').attr("action");
    var inputs = $('#client-type-add :input');
       ...
       .done(function( msg ) {
            ...
            if (msg) {//get server msg
                ...
                if ($.isNumeric(msg)){ //if msg starts numeric, ie last db insert id
                    ...
                    $('#client_types tbody').append('<tr><td>'+name.replace(/[\W-]+/g,"-")+'</td><td>'+description+'</td></tr>');
                    $(form1)[0].reset();
