...
/**********************
 * clicking on "Filter" button in services View page filters popup dialog is opened and inputs are filled from "hidden_" hidden inputs of form and date initialization
 * access public
 * return none
 *********************************/
function servicesListFilterApplied( ) {
    $('.tooltip-inner').css('display', 'none');

    $( "#services_list_dialog_filter" ).modal(  {
        "backdrop": "static",
        "keyboard": true,
        "show": true
    }  );
    $("#filter_sv_title").val( jQuery.trim($("#hidden_filter_sv_title").val()) )
    $("#filter_vendor_type_id").val( jQuery.trim($("#hidden_filter_vendor_type_id").val()) )
    $("#filter_active_status").val( jQuery.trim($("#hidden_filter_active_status").val()) )
 ...
 * return none
 *********************************/
function servicesListMakeFilterDialogSubmit() {
    $("#hidden_filter_sv_title").val( jQuery.trim($("#filter_sv_title").val()) )
    $("#hidden_filter_vendor_type_id").val( jQuery.trim($("#filter_vendor_type_id").val()) )
    $("#hidden_filter_active_status").val( jQuery.trim($("#filter_active_status").val()) )
    ...
    $("#page_number").val(1)
    var theForm = document.getElementById("form_services");
    theForm.submit();
}
